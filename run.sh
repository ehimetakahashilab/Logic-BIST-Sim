...
ln -s ./circuit/${CIRCUIT_NAME}
ln -s ./tpg/lfsr.dat

if [ "$TPG" = "0" ];
then
    rm -f ${CIRCUIT_NAME}_lfsr_pi.dat
    ./tpg/lfsr ${CIRCUIT_NAME} $TEST_VEC ${CIRCUIT_NAME}_lfsr_pi.dat
    ln -s tmp.test ${CIRCUIT_NAME}_lfsr_pi.dat
else
    ln -s ./ATPG/SA/Com1Test/"${CIRCUIT_NAME}".tests ATPG.dat
fi
