...
void phase_shifter(int LFSR[], int nBit) {
  int ia;
  for (ia = 1; ia < nBit; ia++) LFSR[ia] ^= LFSR[ia - 1];
}
...
void InternalLfsrGen(int *Xor) {
  ...
  while (count < TAP_NUM / 2 + 1) {
    ...
    temple = (feedbackbit << (LFSR_BIT - 1)) |
             (TapGen_state[TAP_NUM / 2 + count] >> 1);
    mask = 0;
    for (i = 2; i <= Xor[0]; i++)
...
int cnt;
{
  ...
  for (ia = 16 - 1; ia >= 0; ia--) printf("%d", (seed >> ia) & 0x0001);
  printf("\n");
#elif SEEDMEMORY
  ...
  for (ia = 0; ia < cnt; ia++) fscanf(fin, "%d", &seed);
  fclose(fin);
#endif
...
int ff_state[];
{
  ...
  for (ia = 0; ia <= LFSR_BIT; ia++) {  // initialize SI's LFSR
    if (ia % 4)
      lfsr_state[ia] = 0;
    else
      lfsr_state[ia] = 1;
...
int PeakTogCount(ff_state, chainum) int ff_state[];
{
  ...
  for (ia = schain[chainum].top + 1;
       ia < schain[chainum].top + schain[chainum].length; ia++) {
...
int ff_state[];
{
  ...
  for (ia = 0; ia <= (ffnum - 1) / chainnum; ia++) {
...
#if POWEREVA
      if (ff_state[schain[ib].top + schain[ib].length - ia - 1] !=
          schain[ib].lastval) {
        ...
        if (ia <= schain[ib].length - 2 &&
            Tem_state[schain[ib].top + schain[ib].length - ia - 1] ==
                Tem_state[schain[ib].top + schain[ib].length - ia - 2])
          OrigPatTog[ib]++;
...
#if PEAK
      if (TogPerClk[ib] > ShiftPeak[ib]) ShiftPeak[ib] = TogPerClk[ib];
#endif
      schain[ib].lastval =
          ff_state[schain[ib].top + schain[ib].length - ia - 1];
    }
    ...
    if (ia == 0) {
      ...
      printf("     ");
      for (ib = 0; ib <= LFSR_BIT; ib++) printf("%2d ", ib);
      printf("\n");

      printf("     ");
      for (ib = 0; ib <= LFSR_BIT; ib++) printf("===", ib);
      printf("\n");
    }

    printf("%3d |", ia);

    for (ib = 0; ib <= LFSR_BIT; ib++) printf(" %d ", lfsr_state[ib]);
    printf("\n");
#endif  // LFSR pattern input

    if (clocktime == 1 && ia == 0) {
      // phase_shifter(lfsr_state, LFSR_BIT);
      for (i = 0; i < TAP_NUM; i++) TapGen_state[i] = 0;
      for (i = 0; i < LFSR_BIT; i++)
    ...
    for (ib = 0; ib < LFSR_BIT; ib++)
      lfsr_state[ib] =
          (TapGen_state[TAP_NUM / 2 + 1] >> (LFSR_BIT - ib - 1)) & 1;
