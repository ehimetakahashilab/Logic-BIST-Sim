...
faultsim(argv) char *argv[13];
{
  ...
  float TmpShiftPeek = 0.0, TmpSIPeek = 0.0, TmpSOPeek = 0.0,
        TmpCapPeek[MAXCAP];
  int sum;
  ...
  switch (MODE_TOOL) {
    ...
    case MULTITEST:
      ...
      sprintf(outpath, "./OUTPUTS/MULTI_BIST/%s_%d.csv", basename(argv[1]),
              cap_freq);
      fout_flt_pat = fopen(outpath, "w+");
    ...
    case MULTI_OP:
      ...
      sprintf(outpath, "./OUTPUTS/MULTI_BIST_OB/%s_%d.csv", basename(argv[1]),
              cap_freq);
      fout_flt_pat = fopen(outpath, "w+");
      //			printf("FCOV_PAT output file is not exist!\n"),
      // exit(1);
    ...
    case MULTI_CP:
      ...
      switch (CP_CTR_MODE) {
        case 0:  // Non Toggle gate insertion
          ...
          // cap_freq, SKIP_CYCLE, basename(argv[1]), OBSERVE_RATE,
          // FF_SEL_METHOD, ff_rate);
          break;
        case LCP_TOG:  // toggle gate insert by toggling
          sprintf(cpi_sim_outpath,
                  "./OUTPUTS/CPI/%dcycles/%dSKIP/%s_LCPI_TGL_%d_%.2f.csv",
                  cap_freq, SKIP_CYCLE, basename(argv[1]), INTERVAL_CYCLE,
                  Tgl_rate);
          sprintf(cpi_sim_outpath,
                  "./OUTPUTS/CPI/%d_cycles/%s_LCPI_TGL_%d_%d_%.2f.csv",
                  cap_freq, basename(argv[1]), INTERVAL_CYCLE, SKIP_CYCLE,
                  Tgl_rate);
          break;
        case LCP_RAN:
          sprintf(cpi_sim_outpath,
  ...
  if (MODE_TOOL == MULTI_CP) {
    if (CP_CTR_MODE == FCP_RAN) {  // FF_TPI by LFSR
    ...
    } else if (CP_CTR_MODE == LCP_RAN) {  // Toggle Gate TPI by LFSR
  ...
  //(float)remain_flt / (float)sum_flt) <= 0.90001; time++)
  {
    ...
    for (ia = 0; ia <= cap_freq; ia++) toggle_cap_perpat[ia] = 0;
#endif
    ...
    ///////PI Pattern Set////////////////
    if (TPG_MODE == 1) {  //=1:ATPG, =0:LFSR
      for (ia = 1; ia <= n_inp; ia++) fscanf(test_pat, "%d", &pivalset[ia]);
      for (ia = 0; ia < ffnum; ia++) fscanf(test_pat, "%d", &ff_state[ia]);
    } else {
    ...
    ///////TPI Pattern Set////////////////
    if (MODE_TOOL == MULTI_CP) {
      if (CP_CTR_MODE == FCP_RAN || CP_CTR_MODE == LCP_RAN) {  // TPI by LFSR
        ...
        for (ib = SKIP_CYCLE - 1; ib <= cap_freq; ib++) {
          ...
          // printf("\n");
          for (ia = 0; ia < n_tpi; ia++) tgl_tpi[ia][ib] = tmp_tpi[ia];
        }
    ...
    for (ia = 1; ia <= cap_freq; ia++) {
      ...
      if (MODE_TOOL == MULTI_CP && CP_CTR_MODE == FCP_RAN &&
          ia >= SKIP_CYCLE - 1 && ia < cap_freq) {
...
// printf("Test Power Evaluation \n#SHIFT_IN,  SHIFT_OUT, SHIFT");
...
#endif
      printf("\n");
      printf("%.0lf,%.0lf,%.0lf,%.0lf", (float)toggle_scn_in,
      ...
      printf("%4.3f, %4.3f", (float)toggle_scn / (float)toggle_scn_max * 100.0,
             Max * 100);
#endif
      printf("\n");
#if WSA_REC
...
#if PO_OBSERVE
  for (ia = 0; ia < numout; ia++) free(po_observe[ia]);
  free(po_observe);
#endif
  ...
  if (MODE_TOOL == MULTI_OP || MODE_TOOL == MULTI_CP) {
#if SELECT_STATION
    for (ia = 0; ia < num_observe; ia++) free(ff_observe[ia]);
    free(ff_observe);
#else
    for (ia = 0; ia < ffnum; ia++) free(ff_observe[ia]);
    free(ff_observe);
#endif
  }
  ...
  fclose(test_pat);
  if (MODE_TOOL == MULTI_CP &&
      (CP_CTR_MODE == FCP_RAN || CP_CTR_MODE == LCP_RAN)) {
    fclose(tpi_pat);
    // printf("hehre1?\n");
    for (ia = 0; ia <= n_tpi; ia++) free(tgl_tpi[ia]);
    free(tgl_tpi);
    // printf("hehre2?\n");
  }
#if OUTPUT_FLIST
