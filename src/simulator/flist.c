...
make_line_list(argv) char *argv[13];
{
  ...
  for (ni = lpnt; ni >= 1; ni--) {
    ...
    // printf("%d %d %d %d\n", fnode->line, fnode->type, fnode->innum,
    // fnode->outnum);
...
make_fault_list(address) L_NODE **address;
{
  FIN_NODE *finnode, *head_fin;
...
make_Tranfault_list(address) L_NODE **address;
{
  FIN_NODE *finnode, *head_fin;
  FLT_NODE *head_flt, *flttag;
...
read_fault_list(address, flt_file) L_NODE **address;
char *flt_file;
{
  FIN_NODE *finnode, *head_fin;
  FLT_NODE *head_flt, *flttag;
  int ni, saval, fil, fol, d_time, back, forwd, det_flt_num, det_fltnum2,
      und_fltnum;
  FILE *fp;
...
prn_fltlst(flttag) FLT_NODE *flttag;
{
  for (; flttag != NULL; flttag = flttag->next) {
    /*     printf(" --- Fault node %x  prev %x next
...
prn_fltlst_format(flttag) FLT_NODE *flttag;
{
  int cnt = 0;
  for (; flttag != NULL; flttag = flttag->next) {
...
prn_node(head_node) L_NODE *head_node;
{
  ...
  fnode = head_node;
  for (; fnode != NULL; fnode = fnode->next) {
    // printf(" --- Node[%x] %d Type %d next %x prev
