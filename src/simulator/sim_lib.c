...
update_nextstate(capture) int capture;
{
  ...
  for (ia = 0; finnode != NULL; finnode = finnode->next, ia++) {
    fnode = finnode->node;
    tmp[ia] = fnode->finlst->node->gdval1;
    //  if(fnode->line==202 ||fnode->line==201) printf("ff-%d=
...
{  // just toggle cp
  ...
  for (; fnode != NULL; fnode = fnode->next) {
    ...
    if (capture < SKIP_CYCLE - 1 || fnode->cp_flag != 1 ||
        capture % INTERVAL_CYCLE) {
      continue;
    }
    ...
    for (; finnode != NULL; finnode = finnode->next) {
      ...
      switch (fnode->type) {
        ...
        case NOR:
          ...
          break;
        default:
          fprintf(stderr, " error type %d -284-\n", fnode->type);
          exit(1);
      }
    }
    // printf("output1 :Line %d val %x newval %x \n", fnode->line,
...
//}
    ...
    if (capture < SKIP_CYCLE - 1 || !fnode->cp_flag || capture % INTERVAL_CYCLE)
      continue;
    if (CP_CTR_MODE == LCP_TOG) {
      // tgl_val = tmp2_val ^ fnode->gdval1;
      ...
      //  if(fnode->line == 3931)
      //   printf("%d %d: %x %x %x\n", capture,fnode->line,
      //   fnode->ftval1,fnode->tff_trans_fval,fnode->tff_org_gval);
    } else if (CP_CTR_MODE == LCP_RAN) {
      if (tgl_tpi[tpi_cnt][capture] == 1) {
        fnode->ftval1 = ALL_F;
      } else if (tgl_tpi[tpi_cnt][capture] == 0) {
