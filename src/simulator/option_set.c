...
char *argv[14];
{
  ...
  switch (MODE_TOOL) {
    ...
    case MULTI_OP:   // Multi-cycle test with SEQ-OB
      TPG_MODE = atoi(argv[3]);
      if (TPG_MODE == 0) {
        printf("TPG: %d bit LFSR \n", LFSR_BIT);
      } else {
      ...
      if (MODE_TOOL == 2) {
        ...
        printf(
            "Tool: BIST Multi-Cycle Testing With Sequential Observation "
            "Mode\n");
        if (argc > 8) {
          printf("error: too much arguments!\n");
          printf(
      ...
      for (ia = 0; ia < length + 1; ia++)
        for (ib = 0; ib < ffnum; ib++) {
          ...
          Pat_FF_Faults[ia][ib].OB_Cap_Faults =
              (unsigned int *)calloc(sum_flt / 32 + 1, sizeof(unsigned int));
          Pat_FF_Faults[ia][ib].Total_Faults =
              (unsigned int *)calloc(sum_flt / 32 + 1, sizeof(unsigned int));
          if (Pat_FF_Faults[ia][ib].Last_Cap_Faults == NULL ||
              Pat_FF_Faults[ia][ib].OB_Cap_Faults == NULL ||
              Pat_FF_Faults[ia][ib].Total_Faults == NULL)
...
#else
      printf(
          "Fault Simulation: Disable\n  warning: only Logic Simulation will be "
          "performed\n    if you want Fault simulation, please go setting at "
          "'declare.h'\n\n");
#endif
...
Out_Put(argv) char *argv[13];
{
  ...
  switch (MODE_TOOL) {
    case SCTEST:
      ...
      sprintf(outpath, "./OUTPUTS/BIST/%s.txt", basename(argv[1]));
      ...
      printf("%8d,%8d,%8d,%8d,  %4.4f\n", length, sum_flt, sum_flt - remain_flt,
             remain_flt, (1 - (float)remain_flt / (float)sum_flt) * 100.0);
      fprintf(fout, "%8d,%8d,%8d,%8d,  %4.4f\n", length, sum_flt,
              sum_flt - remain_flt, remain_flt,
...
#endif
      fprintf(fout, "\n");
      fprintf(fout, "%.0lf,%.0lf,%.0lf,%.0lf", (float)toggle_scn_in,
              (float)toggle_scn_out, (float)toggle_scn, (float)toggle_scn_max);
#if PEAK
      float Max = 0;
    ...
    case MULTITEST:
      remain_flt = count_flt(fltlst.next);
      // printf("here?\n"); exit(1);
      sprintf(outpath, "./OUTPUTS/MULTI_BIST/%s.txt", basename(argv[1]));
      if ((fout = fopen(outpath, "w")) == NULL) {
    ...
    case MULTI_OP:
      ...
      sprintf(outpath, "./OUTPUTS/MULTI_BIST_OB/%s.txt", basename(argv[1]));
      if ((fout = fopen(outpath, "w")) == NULL) {
      ...
      char *OPName1 = "Total_Results", *OPName2 = "Last_Cap_Results",
           *OPName3 = "OB_Cap_Results", *OPName4 = "Eve_Cap_Results";
      char out1[50] = {'\0'}, out2[50] = {'\0'}, out3[50] = {'\0'},
           out4[50] = {'\0'}, tmp[10] = {'\0'};
      char *atpg = "ATPG", *lfsr = "LFSR", mode[4] = {'\0'};
      printf("%s\n", mode);
      strcpy(tmp, basename(argv[1]));
      ...
      for (ia = 0; ia <= length; ia++) {
        ...
        for (ib = 0; ib < ffnum; ib++) {
          fprintf(fout, "\nFF %1d %1d", ib,
                  Pat_FF_Faults[ia][ib].Total_Faults[0]);
          if (ia != 0) {
            fprintf(fout1, "\nFF %1d %1d", ib,
                    Pat_FF_Faults[ia][ib].Last_Cap_Faults[0]);
            fprintf(fout2, "\nFF %1d %1d", ib,
                    Pat_FF_Faults[ia][ib].OB_Cap_Faults[0]);
          }
          fprintf(fout3, "\nFF %1d", ib);
          ...
          for (id = 1; id <= sum_flt / 32 + 1; id++) {
            ...
            if (ia != 0) {
              fprintf(fout1, " %1x ",
                      Pat_FF_Faults[ia][ib].Last_Cap_Faults[id]);
      ...
      for (ia = 0; ia < length + 1; ia++) free(Pat_FF_Faults[ia]);
      free(Pat_FF_Faults);
#endif
#endif
    ...
    case MULTI_CP:
      ...
      switch (CP_CTR_MODE) {
        case 0:  // Non Toggle gate insertion
          sprintf(outpath, "./OUTPUTS/CPI/%dcycles/Non_%s.txt", cap_freq,
                  basename(argv[1]));
          break;
        case LCP_TOG:  // toggle gate insert
        case LCP_RAN:

          sprintf(outpath, "./OUTPUTS/CPI/%d_cycles/%s_LogicCPI.txt", cap_freq,
                  basename(argv[1]));
          break;
        case FCP_RAN:  // toggle FF insert
        case FCP_TOG:
          sprintf(outpath, "./OUTPUTS/CPI/%d_cycles/%s_FFCPI.txt", cap_freq,
                  basename(argv[1]));
          break;
      ...
      basename(argv[1]),INTERVAL_CYCLE);break; case 3: sprintf(outpath,
      "./OUTPUTS/TGL_GATE/%dcycles/%s_Struc_Over_%d.txt",cap_freq,
      basename(argv[1]),INTERVAL_CYCLE);break; default: break;
              }
