...
*/
  ...
  for (ia = 1; ia < test_vec; ia++) {
    if (nfin <= max) {
      ...
      tmp_fin = 0;
      for (ic = 0; ic < loop; ic++) {
        for (ib = nfin_max; ib >= 1; ib--) X_LOOP[ib] = X_LOOP[ib - 1];

        for (ib = 1; ib < XOR[0]; ib++) X_LOOP[XOR[ib + 1]] ^= X_LOOP[nfin_max];
        X_LOOP[0] = X_LOOP[nfin_max];

#if PHASE_SHIFT
        for (ib = 0; ib < nfin_max; ib++) Y[ib] = X_LOOP[ib];

        phase_shifter(Y, nfin_max);

        for (ib = 0; ib < nfin_max; ib++) fprintf(fout, " %d", Y[ib]);
#else
